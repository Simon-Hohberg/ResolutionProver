...
public class tptp_tester {
	...
	String includeDir = null; 
	     ...
	     */
	     ...
	     LinkedList<TopLevelItem> allParsed = 
		 new LinkedList<TopLevelItem>();
    ...
    void parseAndCheck(String fileName,
		       ...
		       LinkedList<TopLevelItem> results,
		       int recursionDepth) 
	throws Exception
	{
	    ...
	    for (TopLevelItem item = 
		     (TopLevelItem)parser.topLevelItem(outputManager);
		 item != null;
		 item = (TopLevelItem)parser.topLevelItem(outputManager))
	    {
		if (includeDir != null &&
		...
		{		    
			...
			((IncludeDirective)item).getFileName();
     ...
     */
    public 
    static void checkSemantically(TopLevelItem item,
				  String fileName) 
	throws Exception
	{
	assert item != null;
     ...
     */
    public 
    static void checkSemantically(Formula formula)
	throws Exception
	{
     ...
     */
    public 
    static void checkSemantically(Clause clause) 
	throws Exception
	{
     ...
     */
    public 
    static void checkSemantically(Atomic atom)
	throws Exception
	{
     ...
     */
    public 
    static void checkSemantically(Term term) 
	throws Exception
	{
    ...
    // See Fitting p.13
    public 
	static int formulaDegreeAsInFitting(Formula formula)
    {
	int res = 0;
	switch (formula.getKind()) {	    
	case Atomic: 
	    ...
	    break;
	case Negation: 
            int argdepth = formulaDegreeAsInFitting(((Negation)formula).getArgument());
	    res = 1 + argdepth;
	    break;
	case Binary: 
	    ...
	    break;
	case Quantified: 
	    int matrixdepth = formulaDegreeAsInFitting(((Quantified)formula).getMatrix());
	    res = 1 + matrixdepth;
	    break;
	}
	return res;
    ...
    public 
	static boolean isFalse(Formula formula)
    { 
	switch (formula.getKind()) {
	case Atomic:
	    String pred = ((Atomic)formula).getPredicate();
    ...
    public 
	static boolean isPureAtom(Formula formula)
    { 
	switch (formula.getKind()) {
	case Atomic:
    ...
    // returns true if formula is ~ $true
    public 
	static boolean isNegTrue(Formula formula)
    { 
	switch (formula.getKind()) {
	case Negation:
    ...
    // returns true if formula is ~ $false
    public 
	static boolean isNegFalse(Formula formula)
    { 
	switch (formula.getKind()) {
	case Negation:
	    Formula argument = ((Negation)formula).getArgument();
    ...
    public 
	static boolean isNegAtom(Formula formula)
    { 
	switch (formula.getKind()) {
	case Negation:
	    Formula argument = ((Negation)formula).getArgument();
    ...
    // returns true if formula is ~ A where A is an atom different from $true and $false
    public 
	static Formula getNegAtom(Formula formula)
    { 
	switch (formula.getKind()) {
	case Negation:
	    Formula argument = ((Negation)formula).getArgument();
    ...
    public 
	static boolean isNegNegFormula(Formula formula)
    { 
	switch (formula.getKind()) {
	case Negation:
	    Formula argument = ((Negation)formula).getArgument();
    ...
    public 
	static Formula  getNegNegFormula(Formula formula)
    { 
	switch (formula.getKind()) {
	case Negation:
	    Formula argument = ((Negation)formula).getArgument();
	    switch (argument.getKind()) {
	    case Negation: return ((Negation)argument).getArgument();
	    default: throw new Error("Unexpected case: " + formula.toString());
	    }
	default: throw new Error("Unexpected case: " + formula.toString());
	}
    ...
    public 
	static boolean isAlphaFormula(Formula formula)
    { 
	Formula argument;
    ...
    public 
	static Formula negateFormula(Formula formula)
    {
    ...
    public 
	static Formula getAlpha1(Formula formula)
    { 
	Formula argument;
    ...
    public 
	static Formula getAlpha2(Formula formula)
    { 
	Formula argument;
	Formula returnFormula;
	TptpParserOutput.BinaryConnective connective;
    ...
    public 
	static boolean isBetaFormula(Formula formula)
    { 
	Formula argument;
	TptpParserOutput.BinaryConnective connective;
	switch (formula.getKind()) {
    ...
    public 
	static Formula getBeta1(Formula formula)
    { 
	Formula argument;
	Formula returnFormula;
	TptpParserOutput.BinaryConnective connective;
    ...
    public 
	static Formula getBeta2(Formula formula)
    { 
	Formula argument;
	Formula returnFormula;
	TptpParserOutput.BinaryConnective connective;
    ...
    public 
	static boolean isGammaFormula(Formula formula)
    { 
    ...
    public 
	static Formula getGammaX(Formula formula)
    { 
    ...
    public 
	static boolean isDeltaFormula(Formula formula)
    { 
	TptpParserOutput.Quantifier quantifier;
	Formula argument;
	switch (formula.getKind()) {
    ...
    public 
	static Formula getDeltaX(Formula formula)
    { 
	TptpParserOutput.Quantifier quantifier;
	Formula argument;
	Formula matrix;
    ...
    public 
	static int formulaDepthAsInFitting(Formula formula)
    {
    ...
    public 
	static int formulaRankAsInFitting(Formula formula)
    {
    ...
    // ********************************************************************************* //
    public 
	static int clauseLength(Clause clause)
    {
   ...
   public 
	static Formula 
	substituteFormula ( Formula formula,
			    Term var,
			    Term term, 
			    SimpleTptpParserOutput out )
    { 	
	...
	case Quantified:
	    SimpleTptpParserOutput.Quantifier quantifier = ((Quantified)formula).getQuantifier();
	    ...
	    Formula renamedMatrix = substituteFormula(matrix,boundVariableTerm,newVarTerm,out);
		...
		return (Formula)out.createQuantifiedFormula(quantifier,varlist,newMatrix);
	case Binary:
	    SimpleTptpParserOutput.BinaryConnective connective = ((Binary)formula).getConnective();
    ...
    // ********************************************************************************* //
    public 
	static void demoExample(TopLevelItem item,SimpleTptpParserOutput out)
    {
	...
	case Formula: 
	    ...
	    System.out.println("Equivalent free formula: " + equivalentFreeFormula.toString());
    ...
    /**
     * Replaces all equivalences with implications ((x <=> y) ~> (x => y) ^ (y => x)).
     * The passed formula will not be altered.
     * @param inFormula
    ...
    public static Formula replaceEquivalences(Formula inFormula) {
		...
		case Binary:
			...
			if (binary.getConnective() == TptpParserOutput.BinaryConnective.Equivalence) {
				...
				//actual replacement
				Binary leftImplication = new Binary(leftResult, TptpParserOutput.BinaryConnective.Implication, rightResult);
				Binary rightImplication = new Binary(rightResult, TptpParserOutput.BinaryConnective.Implication, leftResult);
				Binary conjunciton = new Binary(leftImplication, TptpParserOutput.BinaryConnective.And, rightImplication);
		...
		case Negation:
			...
			return new Negation(replaceEquivalences(((Negation)inFormula).getArgument()));
		case Quantified:
    ...
    public static Hashtable<String,SymbolDescriptor> _signature = 
