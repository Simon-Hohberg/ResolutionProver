...
public class SimpleTptpParserOutput implements TptpParserOutput {
	...
	public TptpParserOutput.FofFormula createQuantifiedFormula(
			...
			Iterable<String> variableList, TptpParserOutput.FofFormula formula) {
		assert variableList != null && variableList.iterator().hasNext();
		TptpParserOutput.FofFormula key = formula;
		for (String var : variableList) {
			key = (TptpParserOutput.FofFormula) (new Quantified(quantifier,
					sharedCopyOf(var), key));
		}
		;
	...
	public TptpParserOutput.FofFormula atomAsFormula(
		...
		return (TptpParserOutput.FofFormula) ((Atomic) atom);
	}
	...
	public TptpParserOutput.Source createSourceFromInferenceRecord(
		...
		return new tptp.Source.Inference(sharedCopyOf(inferenceRule),
				usefulInfo, parentInfoList);
	}
	...
	/** Implements the corresponding spec in TptpParserOutput. */
	public TptpParserOutput.InfoItem createDescriptionInfoItem(
			String singleQuoted) {
	...
	public TptpParserOutput.GeneralTerm createGeneralColon(
			TptpParserOutput.GeneralTerm left,
	...
	/**
	 * Reinitialises everything. Note that after a call to reset() objects
	 * created by various method calls prior to that call to reset(), are
	 * considered invalid and should not be used in any way.
	 */
	public void reset() {
		_stringTable.clear();
		_termTable.clear();
	...
	/**
	 * Returns the copy of <strong> str </strong> stored in _stringTable; the
	 * copy is created if necessary.
	 */
