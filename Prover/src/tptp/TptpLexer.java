package tptp;
// $ANTLR 2.7.7 (20120807): "tptp.g" -> "TptpLexer.java"$
...
inputState.guessing--;
	...
	public final void mUNSIGNED_INTEGER(boolean _createToken) throws RecognitionException, CharStreamException, TokenStreamException {
		...
		case '+':
		{
			match('+');
			if ( inputState.guessing==0 ) {
				_ttype = INTEGER;/* it cannot be unsigned integer */
			}
			break;
		}
		case '-':
		...
		if ((LA(1)=='.')) {
			...
			do {
				if (((LA(1) >= '0' && LA(1) <= '9'))) {
					...
					if ( _cnt148>=1 ) { break _loop148; } else {throw new NoViableAltForCharException(LA(1), getFilename(), getLine(), getColumn());}
				}
		...
		if ((LA(1)=='E'||LA(1)=='e')) {
			...
			{
				if ( inputState.guessing==0 ) {
					text.append("+");/* normalization: adding "+" */
				}
				break;
			}
			default:
